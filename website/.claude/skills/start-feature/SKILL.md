...
description: |
  ...
  Phase 1: Codebase understanding (Codex for large analysis, Opus for simple orientation + Claude user interaction).
...
**Feature kickoff skill using Codex-first context management and Agent Teams.**
...
```
Phase 1: UNDERSTAND (Codex-first analysis + Claude Lead)
  Codex analyzes large codebases via a subagent; Opus handles simple orientation; Claude interacts with the user
    ↓
Phase 2: RESEARCH & DESIGN (Agent Teams — Parallel)
  Researcher (subagent → Codex) ←→ Architect (Codex) communicate bidirectionally for research and design
    ↓
...
**Analyze large codebases through Codex while Claude interacts with the user. Use Opus only for simple orientation.**

> Main orchestrator context is precious. Large-scale codebase analysis is delegated to Codex through a general-purpose subagent; simple orientation can stay with Opus.

### Step 1: Analyze Codebase with Codex via Subagent

Use a general-purpose subagent to call Codex for large codebase analysis:
...
Task tool:
  ...
  prompt: |
    ...
    Use Codex to inspect the codebase thoroughly; use direct Glob/Grep/Read only for small follow-up checks.
...
1. **Researcher** — Delegates external research and surveys to Codex
   ...
   How to research:
   - Use Codex for comprehensive research:
...
- **Phase 1**: Codex handles large codebase analysis through a subagent while Claude interacts with the user
- **Phase 2**: Agent Teams bidirectional communication allows Researcher (subagent → Codex) and Architect (Codex) to influence each other
