...
- Detailed analysis intended for saved reports
...
- Task requires external research or survey work.
- Task requires large full-codebase analysis.
- Task includes PDF/image/video/audio analysis.
- Task is code implementation outside UI/UX, animation, design, or layout work.
...
Skip Codex only for simple Q&A, simple codebase orientation, obvious one-file tiny edits, or UI/UX/animation/design/layout work where visual iteration is the main task.
...
```bash
codex exec --model "${CODEX_MODEL:-gpt-5.5}" --config model_reasoning_effort=\"high\" --sandbox read-only "Objective: {brief question}" 2>/dev/null
```
...
/codex:rescue --model "${CODEX_MODEL:-gpt-5.5}" --config model_reasoning_effort=\"high\"-mini --effort high investigate the flaky test
